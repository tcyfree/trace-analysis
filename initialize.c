...
struct ssd_info *initiation(struct ssd_info *ssd)
{
    ...
    FILE *fp=NULL;
    ssd->all_page_count = 0;
    ssd->pop_count = 0;
    ssd->buffer_page[0] = ssd->buffer_page[1] = 0;
    ssd->read_req_trace = ssd->write_req_trace = 0;
    ssd->read_size_trace = ssd->write_size_trace = 0;
...
struct dram_info * initialize_dram(struct ssd_info * ssd)
{
    ...
    dram->buffer->max_page_count=ssd->parameter->dram_capacity/ssd->parameter->page_capacity; //512
    dram->buffer->current_page_count = 0;
