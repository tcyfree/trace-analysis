...
struct ssd_info
{
    ...
    unsigned long all_page_count;
    unsigned long pop_count;
    unsigned long buffer_page[2];
    unsigned long long read_req_trace;
    unsigned long long write_req_trace;
    unsigned long long read_size_trace;
    unsigned long long write_size_trace;
...
{//缓存的节点大小是lpn，里面的成员group是该节点的第一个lsn
    ...
    unsigned int stored;                //（判断扇区是不是在缓存里面，1代表在，0代表不在）indicate the sector is stored in buffer or not. 1 indicates the
